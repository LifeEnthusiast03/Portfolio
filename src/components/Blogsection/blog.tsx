...
import {
  ...
  Brain, Globe, Cpu, Layers, Zap, LucideIcon
} from 'lucide-react';
...
const BLOGS: BlogPost[] = [
  { id: 1, title: 'Building a RAG Pipeline with LangChain and Pinecone', excerpt: 'A deep-dive into retrieval-augmented generation — how to combine vector search with large language models to build intelligent, context-aware applications.', tags: ['Gen AI', 'LangChain', 'Pinecone'], category: 'Gen AI', date: 'Apr 2025', readTime: '8 min read', views: 1240, likes: 89, icon: Brain, color: 'from-purple-600/20 to-pink-600/20', border: 'border-purple-500/20', iconColor: 'text-purple-400', glow: 'rgba(168,85,247,0.15)', status: 'published' },
  { id: 2, title: 'WebSockets vs Server-Sent Events: When to Use Which', excerpt: 'A practical comparison of real-time communication protocols — exploring use cases, trade-offs, and implementation patterns for modern web applications.', tags: ['Node.js', 'WebSockets', 'Backend'], category: 'Backend', date: 'Mar 2025', readTime: '6 min read', views: 870, likes: 62, icon: Globe, color: 'from-blue-600/20 to-cyan-600/20', border: 'border-blue-500/20', iconColor: 'text-blue-400', glow: 'rgba(59,130,246,0.15)', status: 'published' },
  { id: 3, title: 'Mastering React Performance: Memoization Deep Dive', excerpt: 'Understanding useMemo, useCallback, and React.memo — when they help, when they hurt, and how to profile your app to make data-driven decisions.', tags: ['React', 'Performance', 'Frontend'], category: 'Frontend', date: 'Feb 2025', readTime: '7 min read', views: 1530, likes: 114, icon: Zap, color: 'from-yellow-600/20 to-orange-600/20', border: 'border-yellow-500/20', iconColor: 'text-yellow-400', glow: 'rgba(234,179,8,0.15)', status: 'published' },
  { id: 4, title: 'Designing a Scalable REST API with Express & PostgreSQL', excerpt: 'From schema design to middleware patterns — a step-by-step walkthrough for building production-ready REST APIs with proper authentication and error handling.', tags: ['Node.js', 'PostgreSQL', 'API'], category: 'Backend', date: 'Jan 2025', readTime: '10 min read', views: 960, likes: 77, icon: Code2, color: 'from-emerald-600/20 to-teal-600/20', border: 'border-emerald-500/20', iconColor: 'text-emerald-400', glow: 'rgba(52,211,153,0.15)', status: 'published' },
  { id: 5, title: 'Vector Databases Explained: Faiss vs Pinecone vs ChromaDB', excerpt: 'A practical guide to understanding vector databases — comparing the top options for embedding storage and similarity search in production AI systems.', tags: ['Gen AI', 'Vector DB', 'Machine Learning'], category: 'Gen AI', date: 'Dec 2024', readTime: '9 min read', views: 2100, likes: 158, icon: Cpu, color: 'from-violet-600/20 to-indigo-600/20', border: 'border-violet-500/20', iconColor: 'text-violet-400', glow: 'rgba(139,92,246,0.15)', status: 'published' },
  { id: 6, title: 'CSS Grid vs Flexbox: A Complete Visual Guide', excerpt: 'Stop guessing which layout model to use. A visual, example-driven comparison of CSS Grid and Flexbox with real-world layout patterns.', tags: ['CSS', 'Frontend', 'Design'], category: 'Frontend', date: 'Nov 2024', readTime: '5 min read', views: 3400, likes: 231, icon: Layers, color: 'from-pink-600/20 to-rose-600/20', border: 'border-pink-500/20', iconColor: 'text-pink-400', glow: 'rgba(236,72,153,0.15)', status: 'published' },
];
  ...
  return (
    <section className="min-h-screen py-28 px-4 sm:px-6 lg:px-8 bg-transparent relative">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-950/60 border border-blue-500/20 text-blue-400 text-xs font-mono tracking-widest mb-6 shadow-[0_0_30px_rgba(37,99,235,0.12)]">
            <Rss size={12} /><span>Latest Articles</span>
          </div>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black mb-4 leading-tight">
            <span className="bg-gradient-to-r from-white via-gray-200 to-gray-400 bg-clip-text text-transparent">My Blog</span>
          </h1>
        ...
        {/* Featured Post */}
        <motion.div className="mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.6 }}>
            ...
            <div className="relative p-8 md:p-10 flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  <span className="text-xs font-bold uppercase tracking-widest text-purple-400 bg-purple-500/10 border border-purple-500/20 px-3 py-1 rounded-full">⭐ Featured</span>
                  {featured.tags.map(t => (
                    <span key={t} className="text-xs text-gray-500 bg-gray-800/50 border border-gray-700/50 px-2 py-0.5 rounded-full">{t}</span>
                  ))}
                </div>
                <h2 className="text-2xl md:text-3xl font-bold text-white mb-3 group-hover:text-purple-300 transition-colors">{featured.title}</h2>
        ...
        {/* Search + Filter */}
        <motion.div className="flex flex-col sm:flex-row gap-4 mb-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.6 }}>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={selected + search} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"
                  ...
                  >
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      <div className="flex items-start justify-between mb-5">
                        ...
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-lg border ${CATEGORY_COLORS[blog.category] || 'bg-gray-800/50 text-gray-400 border-gray-700'}`}>{blog.category}</span>
                      </div>
                      <h3 className="text-base font-bold text-white mb-2.5 leading-snug group-hover:text-blue-300 transition-colors line-clamp-2">{blog.title}</h3>
                      <p className="text-gray-500 text-sm leading-relaxed mb-5 line-clamp-3 flex-1">{blog.excerpt}</p>
        ...
        {/* Subscribe CTA */}
        <motion.div className="mt-16 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.6 }}>
          <div className="inline-flex flex-col sm:flex-row items-center gap-4 px-8 py-6 rounded-2xl bg-black/50 backdrop-blur-xl border border-gray-800/70 hover:border-blue-500/20 transition-all duration-300">
            ...
            <a href="https://www.linkedin.com/in/sougatasaha/" target="_blank" rel="noopener noreferrer"
              ...
              <Rss size={14} />Follow for Updates
            </a>
          </div>
        </motion.div>
      </div>
