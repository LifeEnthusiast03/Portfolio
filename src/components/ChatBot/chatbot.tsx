...
export default function ChatBot() {
  ...
  const sendMessage = useCallback((text?: string) => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      setMessages(prev => [...prev, { type: 'bot', text: getBotResponse(msg), timestamp: new Date() }]);
    }, delay);
  }, [input]);
  ...
  return (
    <>
      <motion.button
        ...
        aria-label="Toggle chat" id="chatbot-toggle">
        ...
        {isOpen && (
          <motion.div
            ...
            <div className="bg-gradient-to-r from-blue-700 to-violet-700 px-5 py-4 flex items-center justify-between shrink-0">
              ...
              <button onClick={() => setIsOpen(false)} className="text-white/60 hover:text-white transition-colors p-1 rounded-lg hover:bg-white/10"><X size={18} /></button>
            </div>

            {/* Messages */}
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 scrollbar-thin scrollbar-thumb-gray-800">
              {messages.map((msg, i) => (
                <motion.div key={i} className={`flex gap-2.5 ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}
            ...
            <div className="px-4 py-3 border-t border-gray-800/60 bg-[#0d1117] shrink-0">
              <div className="flex gap-2">
                ...
                <motion.button onClick={() => sendMessage()} disabled={!input.trim() || isTyping}
                  ...
                  whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} id="chatbot-send">
                  <Send size={16} />
                </motion.button>
              </div>
